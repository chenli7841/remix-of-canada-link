...
ALTER TABLE public.forwarding_items ADD COLUMN IF NOT EXISTS unit_price_cad numeric(12,2) NOT NULL DEFAULT 0;
UPDATE public.forwarding_items SET unit_price_cad = ROUND(unit_price_cny * 0.19, 2) WHERE unit_price_cad = 0 AND unit_price_cny > 0;
...
BEGIN
  ...
  FOR v_item IN SELECT jsonb_array_elements(v_items) LOOP
    IF (v_item->>'name') IS NOT NULL AND trim(v_item->>'name') <> '' THEN
      ...
      END IF;
      -- CAD is source of truth; CNY is derived
  ...
  -- Auto-set declared value (CAD) = Σ qty × unit_price_cad
