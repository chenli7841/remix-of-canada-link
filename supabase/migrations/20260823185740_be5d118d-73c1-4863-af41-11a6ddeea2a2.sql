CREATE TABLE public.wechat_ai_bindings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  visitor_biz_id text NOT NULL UNIQUE,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  bound_at timestamptz NOT NULL DEFAULT now(),
  last_used_at timestamptz
);
GRANT ALL ON public.wechat_ai_bindings TO service_role;
ALTER TABLE public.wechat_ai_bindings ENABLE ROW LEVEL SECURITY;

CREATE TABLE public.wechat_ai_bind_codes (
  code text PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  expires_at timestamptz NOT NULL,
  used_at timestamptz,
  used_by_visitor_biz_id text
);
...
CREATE TABLE public.ai_forwarding_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  idempotency_key text NOT NULL UNIQUE,
  visitor_biz_id text NOT NULL,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  forwarding_id uuid REFERENCES public.forwarding_orders(id) ON DELETE SET NULL,
  request_no text,
  domestic_tracking_no text,
  source text NOT NULL DEFAULT 'wechat_ai',
  payload jsonb,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
BEGIN
  IF caller_uid IS NULL THEN
    -- Trusted server-side call (service role, e.g. the WeChat AI endpoint):
    -- there is no JWT, so the caller must name the target user explicitly.
    IF session_user = 'service_role' AND _target_user_id IS NOT NULL THEN
      uid := _target_user_id;
    ELSE
