...
-- Quote: include route_code + mode per line
...
-- Place + pay: split orders per route, generate waybills per carton, deduct wallet, create invoice
...
BEGIN
  ...
  -- Pass 1: quote each item, group by route_code
  FOR it IN SELECT jsonb_array_elements(v_items) LOOP
    ...
    SELECT * INTO v_rule FROM public.freight_rules WHERE route_id = v_route.id AND is_active = true
      ORDER BY created_at DESC LIMIT 1;
    SELECT * INTO v_customs FROM public.customs_rules WHERE route_id = v_route.id LIMIT 1;
  ...
  -- Pass 2: create one order per route group with its waybills + items
  ...
  FOREACH v_group_key IN ARRAY COALESCE(v_route_keys, ARRAY[]::text[]) LOOP
    ...
    -- Apply full discount on first order only (keeps total exact)
    v_g_disc := 0;
    IF NOT v_first_disc_applied AND v_disc > 0 THEN
      v_g_disc := v_disc;
      v_first_disc_applied := true;
    END IF;
    ...
    INSERT INTO public.orders(
      ...
      uid, 'shop', v_order_no, 'paid', 'paid', now(),
    ...
    FOR v_line IN SELECT jsonb_array_elements(v_group->'lines') LOOP
      INSERT INTO public.order_items(
        ...
        v_line->>'product_slug',
        v_line->>'sku',
        v_line->>'name', NULL, v_line->>'cover_url',
        (v_line->>'price_cny')::numeric,
        (v_line->>'quantity')::int,
        (v_line->>'subtotal_cny')::numeric,
        v_line->>'mode'
      );
    END LOOP;

    -- Waybills: one per carton (unit). Trigger gen_waybill_row_no fills waybill_no.
    IF v_units > 0 THEN
  ...
  -- Invoice covering all generated orders
  ...
  FOREACH v_order_id IN ARRAY v_order_ids LOOP
    ...
    SELECT v_inv_id, o.id,
           'Order ' || o.order_no,
           o.shipping_cny, o.customs_cny, o.insurance_cny, o.total_cny
    FROM public.orders o WHERE o.id = v_order_id;
  END LOOP;

  -- Deduct wallet balance (trigger apply_wallet_tx updates wallets.balance_cad)
  INSERT INTO public.wallet_transactions(
    user_id, type, amount_cny, amount_cad, fx_rate_cny_to_cad,
    status, channel, note
  ) VALUES (
    uid, 'spend', v_grand_total, v_need_cad, v_fx,
    'completed', 'wallet', 'Shop invoice ' || v_inv_no
  );
