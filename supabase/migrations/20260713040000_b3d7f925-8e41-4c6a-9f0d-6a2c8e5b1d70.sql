-- Points-on-spend: award loyalty points whenever a customer's wallet is
-- deducted for a shop checkout (place_shop_order) or a batch payment
-- (pay_batch), at an admin-configurable rate (app_settings.points_rule).
-- Disabled by default so nothing changes until an admin turns it on.
INSERT INTO public.app_settings(key, value) VALUES
  ('points_rule', '{"enabled": false, "points_per_cad": 1}'::jsonb)
ON CONFLICT (key) DO NOTHING;

CREATE OR REPLACE FUNCTION public.award_points_for_spend(_user_id uuid, _amount_cad numeric)
...
DECLARE
  ...
  v_points integer;
BEGIN
  SELECT COALESCE((value->>'enabled')::boolean, false), COALESCE((value->>'points_per_cad')::numeric, 0)
  INTO v_enabled, v_rate
  FROM public.app_settings WHERE key = 'points_rule';

  IF NOT COALESCE(v_enabled, false) OR COALESCE(v_rate, 0) <= 0 OR COALESCE(_amount_cad, 0) <= 0 THEN
    RETURN 0;
  END IF;

  v_points := floor(_amount_cad * v_rate)::integer;
  IF v_points > 0 THEN
    UPDATE public.profiles SET points = COALESCE(points, 0) + v_points WHERE id = _user_id;
  END IF;
  RETURN v_points;
END $$;
GRANT EXECUTE ON FUNCTION public.award_points_for_spend(uuid, numeric) TO authenticated;

-- place_shop_order: same body as before, plus a points award after the wallet deduction.
...
BEGIN
  ...
  v_points := public.award_points_for_spend(uid, v_need_cad);
  ...
  RETURN jsonb_build_object(
    ...
    'total_cny', v_grand_total, 'paid_cad', v_need_cad, 'points_earned', v_points);
END $function$;

-- pay_batch: same body as before, plus a points award after the wallet deduction.
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true, 'paid_cad', total_cad, 'paid_cny', total_cny, 'invoice_id', v_inv_id, 'invoice_no', v_inv_no, 'points_earned', v_points);
END;
$$;
