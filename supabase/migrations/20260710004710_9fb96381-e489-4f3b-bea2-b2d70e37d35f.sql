-- Replace the single blanket "customs_rate" field with three components that combine to
-- the effective duty rate: MFN tariff + GST + anti-dumping duty. Applies to EVERY line now
-- (personal purchases previously paid zero customs — that's changing per business decision),
-- computed as subtotal_cny × (mfn + gst + anti_dumping), same formula for personal & business.
ALTER TABLE public.products
  ADD COLUMN customs_mfn_rate numeric NOT NULL DEFAULT 0,
  ADD COLUMN customs_gst_rate numeric NOT NULL DEFAULT 0,
  ADD COLUMN customs_antidumping_rate numeric NOT NULL DEFAULT 0;

UPDATE public.products SET customs_mfn_rate = COALESCE(customs_rate, 0);

ALTER TABLE public.products DROP COLUMN customs_rate;
