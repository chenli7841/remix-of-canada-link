-- Restrict detained-package diagnosis and correction to the authenticated customer.
-- Unassigned packages require staff verification before customer self-service.
...
BEGIN
  ...
  -- A tracking-number match alone is not proof of package ownership.
  ...
  FROM (
    ...
    JOIN public.profiles p ON p.id = v_uid AND dp.customer_code = p.customer_code
...
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'authentication required' USING ERRCODE = '42501'; END IF;
  IF _confirmation IS DISTINCT FROM 'CONFIRM_CORRECT_PENDING_TRACKING' THEN RAISE EXCEPTION 'explicit confirmation required'; END IF;
  ...
  PERFORM dp.id FROM public.detained_packages dp
  JOIN public.profiles p ON p.id = v_uid AND dp.customer_code = p.customer_code
  WHERE upper(dp.domestic_tracking_no) = v_new AND dp.status = 'detained'
  FOR UPDATE OF dp;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'active detained number not found for this account; contact support';
  END IF;
