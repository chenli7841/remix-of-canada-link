-- Keep commercial multi-item / multi-box semantics, but avoid recalculating the
-- whole forwarding after every individual waybill insert.
...
-- An older trigger name was left behind by an earlier migration. Both trigger
-- names called the same parent-status function, so every write could recalculate
-- the parent twice.
...
-- Suppress per-row mark/status work only inside place_forwarding. The function
-- performs one final mark-number reconciliation after all boxes are present.
