...
版本：v3.0 对接评审稿｜日期：2026-09-11｜接收方：Shipper（ship-cheap.com）开发团队
...
**v3 修订**：补充了"箱数已知线路 / 箱数未知线路（典型为海运）"两种录单场景的具体契约区别，集中在第 5、6、7、10、13 节；其余条款未变。
...
| sender | 是否必填由线路规则决定 |
| packageCount | 箱数已知线路必填，正整数，等于 packages 长度；箱数未知线路（见 5.1 节）不提交此字段 |
| packages[].clientPackageId | 箱数已知线路必填，同单内稳定、唯一的 ship 箱标识；箱数未知线路不提交 packages，改用订单级别 items（见 5.1 节） |
| packages[].items | 非空数组，name 和正整数 quantity 必填；其他商品字段按线路要求 |
| routeData | 线路扩展资料，无则空对象 |
| remark | 可选业务备注，不是服务方内部备注 |

### 5.1 箱数已知线路与箱数未知线路（海运）

线路是否要求首次录单提供箱数，由该线路 `GET /routes/{routeCode}/order-schema` 返回的 `fields` 决定（是否包含 `box_count` 且 `required=true`）。Shipper 按这个返回结果判断走哪种录单方式，服务方不再另外提供专门的标志字段。

- **箱数已知线路**：Shipper 必须按实际箱子提交 `packageCount` 与 `packages[]`，每箱一个 `clientPackageId`。一箱内允许有多种商品，直接在该箱的 `items` 里列出多条，不得按物品行数量推断箱数、也不得把一箱内的多种商品拆成多箱提交。
- **箱数未知线路（典型为海运）**：服务方现有业务里，这类线路下单时客户本就不需要预先知道会用几个箱子，实际箱数由服务方仓库收货时清点后确定，再按当时点数把已申报的物品数量拆入各箱。因此这类线路 `POST /orders` 请求不提交 `packageCount` 与 `packages[]`，只提交订单级别的 `items[]`（品名 + 数量清单，允许多个品名）；创建成功响应里 `packageCount` 为 `null`、`packages` 为空数组，`labelData` 只包含一条不对应具体箱号的整单参考面单（收件地址、客户号、电话、ship SIP 单号、内件清单、目的地，`waybillNumber` 为 `null`），Shipper 凭这一张面单即可，不需要每箱单独出单。真实箱号要等服务方仓库收货、按箱拆分后，由 Shipper 按国内单号重新查询获得。

箱数未知线路的 `items[]`，字段约定与箱数已知线路 `packages[].items` 相同（`name`、`quantity` 必填，其他按线路要求）；不接受既提交 `packageCount`/`packages` 又提交订单级别 `items` 的混合请求，服务方按线路类型校验，类型不匹配返回 422 VALIDATION_FAILED。
...
| editable、deletable、lockReason、editToken | 当前操作资格、不可操作原因及令牌（判定依据见 6.1 节） |
...
### 6.1 editable / deletable 的判定依据

两类线路的 editable/deletable 都以"是否已经产生真实入库事实"为唯一判定依据，不看是否已经生成箱号：

- **箱数已知线路**：只要该单下任一箱已经过仓库入库扫描，即视为不可整单覆盖修改/删除，只能修改尚未入库的部分（具体粒度见 7 节部分入库限制）。
- **箱数未知线路**：仓库尚未开始收货前 `editable=true`、`deletable=true`；仓库一旦开始收货（即产生入库事实），立即锁定，不支持部分锁定/部分编辑，因为该类订单本来就没有"总箱数"这个分母可供衡量"部分"。

**v3 新增**：`PARTIALLY_INBOUND` 状态只用于箱数已知线路；箱数未知线路在收货完成前只在 `PENDING_INBOUND` 与 `IN_WAREHOUSE`（或服务方现有收货中间态，联调时确认）之间切换，不返回 `PARTIALLY_INBOUND`。
...
**v3 新增**：箱数未知线路（5.1 节）没有 packages，PUT 改为整体替换订单级别 items 数组（不含 clientPackageId 概念），其余字段（routeCode、destination、recipient 等）含义不变；请求体里传 packages 字段视为线路类型不匹配，422 拒绝。
...
**v3 新增**：箱数未知线路（5.1 节）的 labelData 可能包含一条不对应任何 packages[] 条目的整单参考面单（waybillNumber 为 null）；Shipper 不能假设每条 labelData 都能在 packages[] 里找到匹配箱号，也不能因为暂时只有一张面单就认为订单异常。
...
- 逐条核对哪些线路属于箱数已知、哪些属于箱数未知（5.1 节）；确认 order-schema 返回结果与实际录单校验一致，不出现"文档说不用填、服务器却拒绝"或反之。

本文描述接口行为，不规定服务方数据库或后台页面结构。正式发布前双方确认本清单，接口未实现的能力不得宣称已上线。

## 13. 本版补充说明

本版沿用服务方现有集运订单及逐箱号码。箱数已知线路每箱可以包含多种商品；Shipper 按实际箱子提交 packages，不得按物品行数量推断箱数。服务方应保证返回的箱数与实际箱数一致，多箱建单失败不得返回部分可用号码。箱数未知线路（5.1 节）不适用以上箱数一致性承诺——服务方仓库实收箱数可能与 Shipper 预期不同，真实箱数以服务方收货结果为准，Shipper 需在收货完成后重新查询获取。
