...
本文件取代基于错误 .NET 项目生成的所有系统修改意见。它仅是开发指导，本轮没有修改业务代码、数据库或后台页面。配套对外文档为 shipper-api-v2.md。
...
另在现有客户分级增加 ship、总负责人两个选项。此项完全独立，不自动分配客户，不关联客户归属、线路、权限、价格或结算。
...
- sales_user_id：ship 在现有系统的销售结算账户标识。
...
对 ship 使用显式服务端授权线路集合，优先复用现有能表达账户授权的配置。若无合适来源，初期可使用服务端 ship 线路 ID 配置，不强制建立管理页面。线路自身须 is_active 且适用于集运。

可提取一般线路字段验证，但不查询每个 ship 客户的 VIP 可见线路。ship 自己分配客户线路且无需同步。本地只核验 ship 获授权线路及提交资料。

不要将两个新增分级加入任何线路白名单/黑名单，不将 ship 专属客户自动设为 ship 分级。当前 VIP_LEVELS 也可能被线路配置界面枚举：必须追踪所有引用，避免“新增选项”导致新项被默认加入线路或改变既有规则。

数据库 public.vip_level 现为 normal、silver、gold、diamond。增加两个独立值（建议 ship、general_manager，中文显示 ship、总负责人）及类型、标签、颜色映射即可；不得给值附加特殊业务分支。客户分级选项列表和线路权限选项如共用数组，需最小拆分以保持本次新增值纯粹作为客户分级选项，不扩展线路设置联动。此为避免副作用的必要调整，不是页面改版。
...
请在 C:/Users/zeyan/Desktop/代码/SinoCargo-Lovable 实现 ship API。先读 AGENTS.md、本文件和同目录 shipper-api-v2.md。ship SIP 单号直接存 forwarding_orders.domestic_tracking_no，复用 place_forwarding、forwarding_orders、forwarding_items、waybills、profiles、addresses 和现有后台仓库费用逻辑。不得采用此前错误 .NET 项目的文件或字段，不改版页面，不建立第二套业务表，不做扣款或推送。客户分级只增加 ship 和总负责人两个无联动选项。重点核实客户销售映射缺口、Auth 同邮箱独立身份、多物品同箱、数据库原子更新、整单费用去重及 service_role 归属隔离。按文档执行最小实现并验证，不改写 Lovable 已推送历史，不改无关文件，不连接生产试单或自动部署。
