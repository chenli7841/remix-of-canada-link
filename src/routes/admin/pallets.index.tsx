...
function PalletsPage() {
  ...
  return (
    <Page
      title="托盘管理"
      subtitle={q.data ? `共 ${q.data.total} 个托盘` : "加载中…"}
      action={
        <button
    ...
    >
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="托盘号 / 客户号 / 线路 / 批次号 / 目的地"
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/5 bg-white/[0.02]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03] text-left text-[11px] uppercase text-slate-400">
            <tr>
              <th className="px-4 py-2.5">托盘号</th>
              <th>线路</th>
              <th>客户</th>
              <th>目的地</th>
              <th>状态</th>
            ...
            {q.data?.items.map((p: any) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-white/[0.03]">
                  ...
                  <td>
                    <PaymentBadge s={p.payment_status} />
                  </td>
                  <td
                    className="text-sm"
                    title={`实际重量 ${actualW} kg（自身 ${selfW} + 下属 ${childW}）· 体积重 ${volW || 0} kg · 采用 ${p.customer_code ? "自身(A)" : "下属之和(B)"}`}
                  >
                    <div className={`font-mono ${p.has_customer ? "font-bold text-sky-300" : "text-sky-300/60"}`}>
                      本身 {selfChg ? `${selfChg} kg` : "—"}
                    </div>
                    <div className={`font-mono ${p.has_customer ? "text-amber-300/60" : "font-bold text-amber-300"}`}>
                      运单 {childChg ? `${childChg} kg` : "—"}
                    </div>
                  </td>
                  <td
                    className="text-sm font-mono"
                    title={`A 托盘自身计费 = 自身运费 + 关税 + 保险 + 附加费 + 末端派送费\nB 运单合计计费 = 下属运费之和 + 关税 + 保险 + 清关费 + 附加费 + 末端派送费\n当前采用：${p.has_customer ? "A（有客户号）" : "B（无客户号）"}`}
                  >
                    <div className={p.has_customer ? "font-bold text-sky-300" : "text-sky-300/60"}>
                      A 本身 CA${Number(p.with_customer_total_cad ?? 0).toFixed(2)}
                    </div>
                    <div className={p.has_customer ? "text-amber-300/60" : "font-bold text-amber-300"}>
                      B 运单 CA${Number(p.without_customer_total_cad ?? 0).toFixed(2)}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <button onClick={() => onPrint(p.id)} className="mr-2 text-[10px] text-slate-300 hover:text-white">
                      <Printer className="inline h-3 w-3" />
                    </button>
                    <Link to="/admin/pallets/$palletId" params={{ palletId: p.id }} className="text-xs text-brand">
                    ...
                    {canDelete && (
                      <DeleteRowButton
                        label="托盘"
                        name={p.pallet_no}
                        extra="下属箱号/运单的托盘关联会被清空。"
                        onDelete={async () => {
                          await delPallet({ data: { id: p.id } });
                          await qc.invalidateQueries({ queryKey: ["pallets"] });
                        }}
                      />
                    )}
                  </td>
