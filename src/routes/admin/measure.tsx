...
type PalletForm = {
  ...
  height_cm: string;
  weight_kg: string;
  self_length_cm: string;
  self_width_cm: string;
  self_height_cm: string;
  self_weight_kg: string;
  self_volume_m3: string;
  notes: string;
};
...
function MeasurePage() {
  ...
  const onSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setParent(r.parent);
      setParentKind(r.parentKind);
      setParentNo(r.parentNo);
  ...
  const submitPallets = async () => {
    ...
    for (const f of palletForms) {
      ...
      drafts.push({
        ...
        width_cm: num(f.width_cm),
        height_cm: num(f.height_cm),
        weight_kg: num(f.weight_kg),
        self_length_cm: num(f.self_length_cm),
        self_width_cm: num(f.self_width_cm),
        self_height_cm: num(f.self_height_cm),
        self_weight_kg: num(f.self_weight_kg),
        self_volume_m3: num(f.self_volume_m3),
      });
    }
    if (!drafts.length) {
      setMsg({ ok: false, text: "无可加入的运单 / 箱数为 0" });
      return;
    }
    setBusy(true);
    setMsg(null);
    try {
      const r: any = await createPallets({
        data: {
          pallets: drafts,
    ...
    <Page title="量尺称重" subtitle="扫描单号后录入 长×宽×高 / 重量 · 方向键在格子间移动 · Enter 保存并打印面单">
      <div className="space-y-4">
        <form onSubmit={onSearch} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <label className="text-xs font-semibold text-slate-300 inline-flex items-center gap-1.5">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div className="text-sm">
                ...
                <span className="text-slate-200">
                  {parent.route_code ?? "—"} / {parent.destination_code ?? "—"}
                </span>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={onSaveAll}
                  disabled={busy}
                  className="inline-flex items-center gap-1.5 rounded-md bg-emerald-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-500/90 disabled:opacity-50"
                >
            ...
            <div className="mb-3 rounded-lg border border-white/10 bg-white/[0.02] p-3">
              ...
              <div className="flex flex-wrap items-end gap-2">
                ...
                <button onClick={applyBulk} className="rounded-md bg-brand px-3 py-2 text-xs font-semibold text-white">
                  应用
                </button>
                <button
                  onClick={() => setBulk({ length_cm: "", width_cm: "", height_cm: "", weight_kg: "" })}
                      ...
                      {(["length_cm", "width_cm", "height_cm", "weight_kg"] as const).map((k, cIdx) => (
                        <td key={k} className="py-1">
                          <input
                            ...
                            data-nav-idx={`${i}:${cIdx}`}
                            onChange={(e) =>
                              updateRow(i, { [k]: e.target.value === "" ? null : Number(e.target.value) } as any)
                            }
        ...
        {showPallet && (
          <div
            ...
            onClick={() => setShowPallet(false)}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 bg-[#0A0F1A] p-5"
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                <button onClick={() => setShowPallet(false)}>
              ...
              <div className="mb-3 rounded-lg border border-white/10 bg-white/[0.03] p-2 text-xs text-slate-400">
                ...
                <span className="ml-3">
                  线路: <span className="text-slate-200">{parent?.route_code ?? "—"}</span>
                </span>
                <span className="ml-3">
                  目的地: <span className="text-slate-200">{parent?.destination_code ?? "—"}</span>
                </span>
                <span className="ml-3">
                  来源仓: <span className="text-slate-200">{parent?.pickup_warehouse ?? parent?.warehouse ?? "—"}</span>
                </span>
                <span className="ml-3">
                  可分配:{" "}
                  <span className="text-slate-200">{selected.size || rows.filter((r) => !r.pallet_id).length} 单</span>
                </span>
              </div>
              ...
              <div className="space-y-3">
                  ...
                  <div key={i} className="rounded-lg border border-white/10 bg-white/[0.02] p-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      {palletForms.length > 1 && (
                        <button
                          onClick={() => setPalletForms((fs) => fs.filter((_, idx) => idx !== i))}
                    ...
                    <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
                      <label className="text-[11px] text-slate-400">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setPalletForms((fs) =>
                              fs.map((x, idx) => (idx === i ? { ...x, boxCount: e.target.value } : x)),
                            )
                          }
                      ...
                      <label className="text-[11px] text-slate-400">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setPalletForms((fs) =>
                              fs.map((x, idx) => (idx === i ? { ...x, notes: e.target.value } : x)),
                            )
                          }
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-amber-300/80">
                        托盘自身 (决定运费快照 self_freight_cny = 线路规则 × 自身重/体积)
                      </div>
                    ...
                    <div className="mt-1 grid grid-cols-2 gap-2 md:grid-cols-5">
                      {(
                        [
                          ["self_length_cm", "自身长 cm"],
                          ["self_width_cm", "自身宽 cm"],
                          ["self_height_cm", "自身高 cm"],
                            ...
                            onChange={(e) =>
                              setPalletForms((fs) =>
                                fs.map((x, idx) => {
                                  ...
                                  if (L > 0 && W > 0 && H > 0)
                                    next.self_volume_m3 = ((L * W * H) / 1_000_000).toFixed(6);
                                  else if (k === "self_length_cm" || k === "self_width_cm" || k === "self_height_cm")
                                    next.self_volume_m3 = "";
                                  return next;
                                }),
                              )
                      ...
                      {(
                        ...
                        <label key={k} className="text-[11px] text-slate-400">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setPalletForms((fs) =>
                                fs.map((x, idx) => (idx === i ? { ...x, [k]: e.target.value } : x)),
                              )
                            }
                            className="mt-1 w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-sm text-slate-100"
                          />
                ...
                <button
                  onClick={() => setPalletForms((fs) => [...fs, emptyPallet()])}
