...
function CartonDetail() {
  ...
  const logsQ = useQuery({
    queryKey: ["carton-logs", cartonId],
    queryFn: () => fetchLogs({ data: { entity_type: "carton", entity_id: cartonId, pageSize: 50 } }),
  });
  ...
  const onPrint = async () => {
    const d = await fetchLabel({ data: { kind: "carton", id: cartonId } });
    ...
    <div className="mx-auto max-w-7xl space-y-4 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-400">
            状态：{carton.status} · 付款 {carton.payment_status} · 创建 {fmtDate(carton.created_at)} · 计费重{" "}
            <span className="font-mono text-amber-300">{chargeableW ?? 0} kg</span>（
            {carton.customer_code ? "自身" : "下属之和"}）
            {carton.batch_status ? ` · 批次状态 ${carton.batch_status}` : ""}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {carton.batch_status && carton.batch_status !== "draft" && (
            <button
              onClick={async () => {
                await setUnlock({ data: { kind: "carton", id: cartonId, unlocked: !carton.unlocked } });
                invalidate();
              }}
          ...
          <button
            onClick={async () => {
              if (confirm("删除此箱号？运单/订单关联会被清空")) {
                await del({ data: { id: cartonId } });
                navigate({ to: "/admin/cartons" });
              }
            }}
      ...
      {locked && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200 inline-flex items-center gap-2">
          ...
          此箱号所属批次为 <b>{carton.batch_status}</b>（非草稿），字段已锁定为只读。如需修改请点击右上角 "人工解锁"。
        </div>
      )}
      ...
      <ContainerEditPanel
        ...
        locked={locked}
        onSave={async (patch) => {
          await update({ data: { id: cartonId, patch } });
          invalidate();
        }}
      />
      ...
      >
        <WaybillCompactList
          ...
          onKick={async (w) => {
            await assign({ data: { cartonId: null, waybillIds: [w.id] } });
            invalidate();
          }}
        />
      </Card>
...
export function SelfDimsCard({
  ...
  const L = Number(f.self_length_cm || 0),
    W = Number(f.self_width_cm || 0),
    H = Number(f.self_height_cm || 0);
  ...
  return (
    <Card
      title={`基本信息 · ${kind === "carton" ? "箱号" : "托盘"}自身尺寸/重量`}
      action={
        <button
          onClick={handleSave}
    ...
    >
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6 text-xs">
        ...
        <ReadOnlyField
          label="自身运费 (CA$)"
          value={selfFreightCad != null ? `CA$${selfFreightCad.toFixed(2)}` : "—"}
          hint="按线路规则自动计算（保存尺寸/重量后即时更新）"
        />
      </div>
      <div className="mt-2 text-[10px] text-slate-500">
        {locked ? "已锁定，字段只读。" : "体积与运费由系统根据所属线路的运费规则自动计算，保存尺寸/重量后立即更新。"}
      </div>
    </Card>
  );
}
...
function ReadOnlyField({ label, value, hint }: { label: string; value: any; hint?: string }) {
  return (
    <div>
      ...
      <div
        title={hint}
        className="mt-1 w-full rounded-md border border-dashed border-white/10 bg-white/[0.03] px-2 py-1.5 text-sm font-mono text-amber-300"
      >
        {value}
      </div>
    </div>
