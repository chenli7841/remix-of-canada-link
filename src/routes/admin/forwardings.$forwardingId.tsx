...
import {
  getForwardingDetail,
  intakeForwarding,
  addWaybillsToForwarding,
  getLabelData,
  previewForwardingFreight,
  setForwardingInsured,
  updateForwardingBasicInfo,
...
function FwDetail() {
  ...
  const canIntake = (meQ.data?.roles ?? []).some((r) =>
    ["owner", "manager", "warehouse_cn", "warehouse_ca"].includes(r),
  );
  ...
  useEffect(() => {
    ...
    foData?.id,
    foData?.warehouse,
    foData?.shipping_method,
    foData?.destination_code,
    foData?.route_code,
    foData?.domestic_tracking_no,
    foData?.intl_tracking_no,
  ]);
  const basicDirty =
    !!foData &&
    (BASIC_FIELDS.some((f) => (basic[f] ?? "") !== (foData[f] ?? "")) || basicRoute !== (foData.route_code ?? ""));
  const saveBasic = async () => {
    ...
    try {
      if (foData && basicRoute && basicRoute !== (foData.route_code ?? "")) {
        await changeRoute({
          data: { entityType: "forwarding", entityId: id, newRouteCode: basicRoute, note: "基础信息修改线路" },
        });
      }
      await saveBasicFn({
        data: { forwardingId: id, patch: Object.fromEntries(BASIC_FIELDS.map((f) => [f, basic[f] ?? ""])) as any },
      });
    ...
    const t = setTimeout(async () => {
      try {
        const p = await preview({
          data: { id, route_id: routeId, declared_value_cad: decl ? Number(decl) : undefined },
        });
        setPreviewData(p);
      } catch {
        setPreviewData(null);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRouteChange && selectedRouteCode) {
        await changeRoute({
          data: { entityType: "forwarding", entityId: id, newRouteCode: selectedRouteCode, note: "入库时变更线路" },
        });
      }
      await intake({
        data: {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-400">
            {fo.intake_at ? (
              <span className="text-emerald-300">已入库 · {fmtDate(fo.intake_at)}</span>
            ) : (
              <span className="text-amber-300">待入库</span>
            )}
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="space-y-1 text-xs text-slate-300">
            {canIntake ? (
              <div className="space-y-2">
                <BasicField
                  label="仓库"
                  value={basic.warehouse ?? ""}
                  onChange={(v) => setBasic((b) => ({ ...b, warehouse: v }))}
                />
                <div className="flex items-center gap-2">
                  ...
                  >
                    {Object.keys(METHOD_LABEL).map((k) => (
                ...
                <BasicField
                  label="国内单号"
                  value={basic.domestic_tracking_no ?? ""}
                  onChange={(v) => setBasic((b) => ({ ...b, domestic_tracking_no: v }))}
                  mono
                />
                <BasicField
                  label="国际单号"
                  value={basic.intl_tracking_no ?? ""}
                  onChange={(v) => setBasic((b) => ({ ...b, intl_tracking_no: v }))}
                  mono
                />
              ...
              <>
                ...
                <div>
                  线路：<span className="font-mono">{fo.route_code ?? "—"}</span>
                  {fo.route_name ? <span className="ml-1 text-slate-400">· {fo.route_name}</span> : null}
                </div>
                <div>目的地：{fo.destination_code ?? "—"}</div>
                <div>
                  批次号：<span className="font-mono">{fo.batch_no ?? "—"}</span>
                </div>
                <div>
                  国内单号：<span className="font-mono">{fo.domestic_tracking_no ?? "—"}</span>
                </div>
                <div>
                  国际单号：<span className="font-mono">{fo.intl_tracking_no ?? "—"}</span>
                </div>
              </>
            )}
            {(() => {
        ...
        <Card title="物品声明">
          <InsuranceBlock
            ...
            canEdit={canIntake}
            onSave={async (next) => {
              await setInsured({ data: { id, insured: next } });
              await qc.invalidateQueries({ queryKey: ["admin-fo", id] });
            }}
          />
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          {shippingAddress ? (
            <div className="space-y-0.5 text-xs text-slate-300">
              <div className="text-slate-100">
                {shippingAddress.recipient ?? shippingAddress.name ?? "—"} · {shippingAddress.phone ?? "—"}
              </div>
              <div>
                {[
        ...
        >
          {user?.reg_address || user?.reg_city || user?.reg_country ? (
            <div className="space-y-0.5 text-xs text-slate-300">
              <div className="text-slate-100">
                {user.full_name ?? "—"} · {user.reg_phone ?? user.phone ?? "—"}
              </div>
              <div>
                {[user.reg_address, user.reg_city, user.reg_province, user.reg_postal_code, user.reg_country]
                  .filter(Boolean)
                  .join(", ")}
              </div>
            </div>
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            <div className="grid gap-2 sm:grid-cols-3">
              ...
              <div>
                ...
                <input
                  readOnly
                  value={previewData?.total_weight?.toFixed(3) ?? "—"}
              ...
              <div>
                ...
                <input
                  readOnly
                  value={previewData ? (previewData.total_volume_cm3 / 1e6).toFixed(4) : "—"}
                  className="w-full cursor-not-allowed rounded-md border border-white/10 bg-white/[0.03] px-2 py-1.5 text-xs text-slate-300"
                />
              </div>
            </div>
            ...
            {previewData?.snapshot && (
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3">
                <div className="mb-2 text-xs font-semibold text-emerald-300 inline-flex items-center gap-1">
                  <Calculator className="h-3 w-3" />
                  运费明细预览
                </div>
                <div className="grid gap-2 text-xs sm:grid-cols-3 md:grid-cols-6">
                  <div>
                    <div className="text-slate-500">实重</div>
                    <div>{previewData.snapshot.actual_weight} kg</div>
                  </div>
                  <div>
                    <div className="text-slate-500">体积重</div>
                    <div>{previewData.snapshot.volumetric_weight} kg</div>
                  </div>
                  <div>
                    <div className="text-slate-500">计费重</div>
                    <div className="text-emerald-300">{previewData.snapshot.chargeable_weight} kg</div>
                  </div>
                  <div>
                    <div className="text-slate-500">单价</div>
                    <div>{fmtCAD(previewData.snapshot.unit_price_cad || 0)}/kg</div>
                  </div>
                  <div>
                    <div className="text-slate-500">运费</div>
                    <div className="text-emerald-300">{fmtCAD(previewData.snapshot.freight_cad || 0)}</div>
                  </div>
                  <div>
                    <div className="text-slate-500">关税</div>
                    <div>
                      {previewData.snapshot.customs_applies === false ? (
                        <span className="text-emerald-300">包关税 / Include</span>
                      ) : Number(previewData.snapshot.duty_cad || 0) > 0 ? (
                        fmtCAD(previewData.snapshot.duty_cad)
                      ) : (
                        "—"
                      )}
                    </div>
                  ...
                  <div>
                    <div className="text-slate-500">保费</div>
                    <div>{fmtCAD(previewData.snapshot.insurance_cad || 0)}</div>
                  </div>
                </div>
            ...
            <button
              type="submit"
              disabled={busy || !previewData?.snapshot}
            ...
            {isRouteChange && (
              <div className="text-[10px] text-amber-400">
                线路与客户选择不同，提交将生成新的集运单号 / 运单号 / 面单。旧号会自动加入历史单号列表。
              </div>
            )}
      ...
      {snap && (
        <Card title="当前运费快照">
          <div className="grid gap-2 text-xs sm:grid-cols-3 md:grid-cols-6">
            <div>
              <div className="text-slate-500">实重</div>
              <div>{snap.actual_weight} kg</div>
            </div>
            <div>
              <div className="text-slate-500">体积重</div>
              <div>{snap.volumetric_weight} kg</div>
            </div>
            <div>
              <div className="text-slate-500">计费重</div>
              <div className="text-slate-100">{snap.chargeable_weight} kg</div>
            </div>
            <div>
              <div className="text-slate-500">运费</div>
              <div className="text-emerald-300">{fmtCAD(snap.freight_cad || 0)}</div>
            </div>
            <div>
              <div className="text-slate-500">关税</div>
              <div>
                {snap.customs_applies === false ? (
                  <span className="text-emerald-300">包关税 / Include</span>
                ) : Number(snap.duty_cad || 0) > 0 ? (
                  fmtCAD(snap.duty_cad)
                ) : (
                  "—"
                )}
              </div>
            ...
            <div>
              <div className="text-slate-500">保费</div>
              <div>{fo.insured ? fmtCAD(snap.insurance_cad || 0) : <span className="text-slate-500">未购买</span>}</div>
            </div>
            <div>
              <div className="text-slate-500">附加费</div>
              <div>{fmtCAD(snap.surcharges_cad || 0)}</div>
            </div>
            <div className="col-span-2 md:col-span-3">
              <div className="text-slate-500">合计（列表"费用"）</div>
              <div className="text-base font-bold text-amber-300">{fmtCAD(snap.total_cad || 0)}</div>
            </div>
          </div>
          <div className="mt-2 text-[10px] text-slate-500">
            公式：合计 = 运费 + 关税 + (已购买ⓘ保险时) 保费 + 附加费(CNY×汇率)
          </div>
        </Card>
      )}
          ...
          {(events ?? []).length === 0 ? (
            ...
            <TrackingTimeline
              events={(events ?? []).map((e: any) => ({
                ...
                status_en: e.status_en,
                location_zh: e.location_zh,
                location_en: e.location_en,
                event_time: e.event_time,
                source:
                  e.source === "admin" ? "admin_action" : e.source === "third_party" ? "third_party" : "admin_manual",
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto">
          ...
          {logs.map((l: any) => {
            const ACTION_LABEL: Record<string, string> = {
              change_route: "变更线路 / 目的地",
              update_dims: "编辑运单尺寸/重量",
              intake: "入库",
              add_waybills: "新增运单",
              set_insured: "保险状态变更",
            };
              ...
              <div key={l.id} className="rounded-md border border-white/5 bg-white/[0.02] p-2 text-xs">
                ...
                {isRoute && l.before && l.after && (
                  <div className="mt-1 grid gap-1 rounded bg-white/[0.03] p-1.5 sm:grid-cols-2">
                    <div>
                      <div className="text-[10px] text-slate-500">变更前</div>
                      <div className="font-mono text-rose-300">{l.before.no ?? "—"}</div>
                      <div className="text-slate-400">
                        线路 {l.before.route_code ?? "—"} · 目的地 {l.before.destination_code ?? "—"}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-500">变更后</div>
                      <div className="font-mono text-emerald-300">{l.after.no ?? "—"}</div>
                      <div className="text-slate-400">
                        线路 {l.after.route_code ?? "—"} · 目的地 {l.after.destination_code ?? "—"}
                        {typeof l.after.waybills_changed === "number"
                          ? ` · ${l.after.waybills_changed} 个运单同步换号`
                          : ""}
                      </div>
                    </div>
                  </div>
                ...
                {isDims && (l.before || l.after) && (
                  <div className="mt-1 grid gap-1 rounded bg-white/[0.03] p-1.5 sm:grid-cols-2">
                    <div>
                      <div className="text-[10px] text-slate-500">变更前</div>
                      <div className="font-mono text-rose-300">{fmtDims(l.before)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-500">变更后</div>
                      <div className="font-mono text-emerald-300">{fmtDims({ ...l.before, ...l.after })}</div>
                    </div>
                  </div>
                )}
              </div>
...
const fmtDims = (v: any) =>
  v ? `${v.weight_kg ?? "—"}kg · ${v.length_cm ?? "—"}×${v.width_cm ?? "—"}×${v.height_cm ?? "—"}` : "—";

function WaybillsSection({
  waybills,
  canEdit,
  onAdd,
  onEditDims,
  ...
  const dup = (n: number) => {
    const last = rows[rows.length - 1] ?? {};
    const add = Array.from({ length: n }, () => ({ ...last }));
    setRows([...rows, ...add]);
  };
  ...
  const submit = async () => {
    setBusy(true);
    try {
      await onAdd(rows.filter((r) => r.weight_kg || r.length_cm));
      setShow(false);
      setRows([{}]);
    } finally {
      setBusy(false);
  ...
  const startEdit = (w: any) => {
    ...
    setEditForm({
      ...
      length_cm: w.length_cm ?? undefined,
      width_cm: w.width_cm ?? undefined,
      height_cm: w.height_cm ?? undefined,
    });
  };
  const saveEdit = async () => {
    ...
    setBusy(true);
    try {
      await onEditDims(editId, editForm);
      setEditId(null);
    } finally {
      setBusy(false);
    }
                ...
                {editId === w.id ? (
                  <>
                    <td>
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm({ ...editForm, weight_kg: e.target.value ? +e.target.value : undefined })
                        }
                      ...
                      <div className="flex gap-1">
                        {(["length_cm", "width_cm", "height_cm"] as const).map((k) => (
                          <input
                            ...
                            onChange={(e) =>
                              setEditForm({ ...editForm, [k]: e.target.value ? +e.target.value : undefined })
                            }
                    ...
                    <td className="text-xs">
                      {snap.customs_applies === false ? (
                        <span className="text-emerald-300">包关税</span>
                      ) : Number(w.duty_cad ?? 0) > 0 ? (
                        fmtCAD(Number(w.duty_cad))
                      ) : (
                        "—"
                      )}
                    </td>
                    <td className="text-xs">
                      {Number(w.insurance_cad ?? 0) > 0 ? fmtCAD(Number(w.insurance_cad)) : "—"}
                    </td>
                    <td className="text-xs">
                    ...
                    <td>
                      <button
                        onClick={saveEdit}
                        disabled={busy}
                        className="mr-1 rounded bg-brand px-2 py-0.5 text-[10px] text-white"
                      >
                        保存
                      </button>
                      <button
                        onClick={() => setEditId(null)}
                        className="rounded border border-white/10 px-2 py-0.5 text-[10px]"
                      >
                        取消
                      </button>
                    </td>
                  ...
                  <>
                    ...
                    <td
                      ...
                      title={
                        snap.actual_weight != null
                          ? `实重 ${snap.actual_weight}kg · 体积重 ${snap.volumetric_weight}kg`
                          : ""
                      }
                    >
                      {cw > 0 ? `${cw} kg` : "—"}
                    ...
                    <td>
                      {canEdit && (
                        <button onClick={() => startEdit(w)} className="rounded p-1 text-slate-400 hover:bg-white/10">
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
          ...
          >
            ...
            <div className="mb-3 flex gap-2 text-xs">
              <button onClick={addBlank} className="rounded-md border border-white/10 px-2 py-1 hover:bg-white/5">
                <Plus className="inline h-3 w-3" />
                添加一行
              </button>
              <button onClick={copyLast} className="rounded-md border border-white/10 px-2 py-1 hover:bg-white/5">
                <Copy className="inline h-3 w-3" />
                复制上一行
              </button>
              <button onClick={() => dup(5)} className="rounded-md border border-white/10 px-2 py-1 hover:bg-white/5">
                +5 同规格
              </button>
              <button onClick={() => dup(10)} className="rounded-md border border-white/10 px-2 py-1 hover:bg-white/5">
                +10 同规格
              </button>
            </div>
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <input
                          ...
                          step="0.001"
                          value={r.weight_kg ?? ""}
                          onChange={(e) => update(i, { weight_kg: e.target.value ? +e.target.value : undefined })}
                          className="w-16 rounded border border-white/10 bg-white/5 px-1 py-1"
                        />
                      </td>
                      <td>
                        <input
                          type="number"
                          value={r.length_cm ?? ""}
                          onChange={(e) => update(i, { length_cm: e.target.value ? +e.target.value : undefined })}
                          className="w-14 rounded border border-white/10 bg-white/5 px-1 py-1"
                        />
                      </td>
                      <td>
                        <input
                          type="number"
                          value={r.width_cm ?? ""}
                          onChange={(e) => update(i, { width_cm: e.target.value ? +e.target.value : undefined })}
                          className="w-14 rounded border border-white/10 bg-white/5 px-1 py-1"
                        />
                      </td>
                      <td>
                        <input
                          type="number"
                          value={r.height_cm ?? ""}
                          onChange={(e) => update(i, { height_cm: e.target.value ? +e.target.value : undefined })}
                          className="w-14 rounded border border-white/10 bg-white/5 px-1 py-1"
                        />
                      </td>
                      <td>
                        <input
                          ...
                          className="w-full rounded border border-white/10 bg-white/5 px-1 py-1"
                        />
                      </td>
                      <td>
                        <button onClick={() => remove(i)} className="text-rose-400">
                          ×
                        </button>
                      </td>
                    </tr>
            ...
            <button
              disabled={busy}
              onClick={submit}
              className="mt-3 w-full rounded-md bg-brand py-2 text-sm font-semibold text-white disabled:opacity-50"
            >
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold text-white">
                运单附加费 · <span className="font-mono text-brand">{surchargeWb.waybill_no}</span>
              </h2>
              <button onClick={() => setSurchargeWb(null)}>
                <X className="h-4 w-4 text-slate-400" />
              </button>
            </div>
            <div className="[&_*]:!text-white [&_.text-slate-500]:!text-white/60 [&_.text-slate-400]:!text-white/70 [&_.text-slate-600]:!text-white/40">
      ...
      {canEdit && (
        <div className="flex items-center gap-2 pt-1">
          ...
          <button
            ...
            className="rounded-md bg-brand px-2 py-1 text-[11px] font-semibold text-white disabled:opacity-40"
          >
            保存
          </button>
        </div>
...
function ItemsCustomerCard({ items }: { items: any[] }) {
  ...
  const fmtCell = (c: (typeof COLS)[number], v: any) => {
    ...
    if (c.k === "unit_price_cad" || c.k === "subtotal_cad")
      return <span className="text-emerald-300">C${Number(v).toFixed(2)}</span>;
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="text-left text-[10px] uppercase text-slate-500">
            <tr>
              {COLS.map((c) => (
                <th key={c.k} className="py-1.5 pr-3 font-medium">
                  {c.label}
                </th>
              ))}
            </tr>
