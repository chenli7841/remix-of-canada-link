...
function CartonsPage() {
  ...
  return (
    <Page
      title="箱号管理"
      subtitle={q.data ? `共 ${q.data.total} 个箱` : "加载中…"}
      action={
        <button
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/5 bg-white/[0.02]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03] text-left text-[11px] uppercase text-slate-400">
            <tr>
              <th className="px-4 py-2.5">箱号</th>
              <th>线路</th>
              <th>客户</th>
              <th>目的地</th>
              <th>状态</th>
              ...
              <th>
                计费重
                <div className="normal-case text-[10px] text-slate-500">
                  <span className="text-sky-300/70">本身</span> / <span className="text-amber-300/70">运单合计</span>
                </div>
              </th>
              <th>
                总费用 (CAD)
                <div className="normal-case text-[10px] text-slate-500">
                  <span className="text-sky-300/70">A 本身</span> /{" "}
                  <span className="text-amber-300/70">B 运单合计</span>
                </div>
              </th>
              <th>所属托盘</th>
              <th>所属批次</th>
            ...
            {q.data?.items.map((c: any) => {
              ...
              return (
                <tr key={c.id} className="hover:bg-white/[0.03]">
                  ...
                  <td>
                    <PaymentBadge s={c.payment_status} />
                  </td>
                  <td className="text-sm" title={`采用 ${c.has_customer ? "本身计费重(A)" : "运单合计计费重(B)"}`}>
                    <div className={`font-mono ${c.has_customer ? "font-bold text-sky-300" : "text-sky-300/60"}`}>
                      本身 {selfChg ? `${selfChg} kg` : "—"}
                    </div>
                    <div className={`font-mono ${c.has_customer ? "text-amber-300/60" : "font-bold text-amber-300"}`}>
                      运单 {childChg ? `${childChg} kg` : "—"}
                    </div>
                  </td>
                  <td
                    className="text-sm font-mono"
                    title={`A 本身计费 = 自身运费 CA$${Number(c.self_freight_cad ?? 0).toFixed(2)} + 关税 CA$${Number(c.child_customs_cad ?? 0).toFixed(2)} + 保险 CA$${Number(c.child_insurance_cad ?? 0).toFixed(2)} + 附加费 CA$${Number(c.surcharge_cad ?? 0).toFixed(2)}\nB 运单合计 = 下属运费 CA$${Number(c.child_freight_cad ?? 0).toFixed(2)} + 关税 + 保险 + 清关费 CA$${Number(c.clearance_fee_cad ?? 0).toFixed(2)}\n当前采用：${c.has_customer ? "A（有客户号）" : "B（无客户号）"}`}
                  >
                    <div className={c.has_customer ? "font-bold text-sky-300" : "text-sky-300/60"}>
                      A 本身 CA${Number(c.with_customer_total_cad ?? 0).toFixed(2)}
                    </div>
                    <div className={c.has_customer ? "text-amber-300/60" : "font-bold text-amber-300"}>
                      B 运单 CA${Number(c.without_customer_total_cad ?? 0).toFixed(2)}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <button onClick={() => onPrint(c.id)} className="mr-2 text-[10px] text-slate-300 hover:text-white">
                      <Printer className="inline h-3 w-3" />
                    </button>
                    <Link to="/admin/cartons/$cartonId" params={{ cartonId: c.id }} className="text-xs text-brand">
                    ...
                    {canDelete && (
                      <DeleteRowButton
                        label="箱号"
                        name={c.carton_no}
                        extra="下属运单/订单的箱号关联会被清空。"
                        onDelete={async () => {
                          await delCarton({ data: { id: c.id } });
                          await qc.invalidateQueries({ queryKey: ["cartons"] });
                        }}
                      />
                    )}
                  </td>
