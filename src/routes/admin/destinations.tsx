...
import { listDestinations, upsertDestination, deleteDestination } from "@/lib/presets.functions";
import { Page } from "@/lib/admin-shared";
import { Plus, Pencil, Trash2, X, Loader2 } from "lucide-react";

export const Route = createFileRoute("/admin/destinations")({ component: DestPage });

function DestPage() {
  const qc = useQueryClient();
  const fetchList = useServerFn(listDestinations);
  const save = useServerFn(upsertDestination);
  const del = useServerFn(deleteDestination);
  const q = useQuery({ queryKey: ["destinations"], queryFn: () => fetchList() });
  const [editing, setEditing] = useState<any | null>(null);
  const [show, setShow] = useState(false);

  return (
    <Page title="目的地" subtitle="后台预设；批次创建时通过下拉选择"
      ...
      <div className="overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03] text-left text-[11px] uppercase text-slate-400">
            <tr><th className="px-4 py-2.5">代码</th><th>中文</th><th>英文</th><th>国家</th><th>排序</th><th>状态</th><th></th></tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {q.isLoading && <tr><td colSpan={7} className="py-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-slate-500"/></td></tr>}
            {q.data?.items.map((c: any) => (
              <tr key={c.id} className="hover:bg-white/[0.03]">
                ...
                <td className="text-xs">{c.country ?? "—"}</td>
                ...
                <td className="px-4 py-2.5 text-right">
                  <button onClick={() => { setEditing(c); setShow(true); }} className="mr-2 text-xs"><Pencil className="inline h-3 w-3"/></button>
                  <button onClick={async () => { if (confirm("删除？")) { await del({ data: { id: c.id } }); qc.invalidateQueries({ queryKey: ["destinations"] }); } }}
                    className="text-xs text-rose-400"><Trash2 className="inline h-3 w-3"/></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {show && <EditModal initial={editing} onClose={() => setShow(false)} onSave={async (p) => {
        await save({ data: p }); setShow(false); qc.invalidateQueries({ queryKey: ["destinations"] });
      }}/>}
    </Page>
  );
}
...
function EditModal({ initial, onClose, onSave }: { initial: any; onClose: () => void; onSave: (p: any) => Promise<void> }) {
  const [form, setForm] = useState({
    ...
    name_en: initial?.name_en ?? "", country: initial?.country ?? "",
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
      <form onClick={(e) => e.stopPropagation()} onSubmit={async (e) => { e.preventDefault(); setBusy(true); await onSave(form); setBusy(false); }}
        ...
        <div className="mb-3 flex items-center justify-between"><h2 className="font-display text-lg font-bold">{initial ? "编辑" : "新增"}目的地</h2>
          <button type="button" onClick={onClose}><X className="h-4 w-4 text-slate-400"/></button></div>
        <div className="space-y-3">
          <Field label="代码 *（如 YYZ）"><input required value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} className={input}/></Field>
          ...
          <Field label="国家代码"><input value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value.toUpperCase() })} className={input}/></Field>
