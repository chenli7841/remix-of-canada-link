...
function PalletDetail() {
  ...
  const logsQ = useQuery({
    queryKey: ["pallet-logs", palletId],
    queryFn: () => fetchLogs({ data: { entity_type: "pallet", entity_id: palletId, pageSize: 50 } }),
  });
  ...
  const cartonsQ = useQuery({
    queryKey: ["cartons-for-pallet"],
    queryFn: () => fetchCartons({ data: {} }),
    enabled: showAssign,
  });
  const wbsQ = useQuery({
    queryKey: ["wbs-for-pallet", wbSearch],
    queryFn: () => fetchWaybills({ data: { search: wbSearch, pageSize: 50, status: "all" } }),
    enabled: showAddWb,
  });
  ...
  const onPrint = async () => {
    const d = await fetchLabel({ data: { kind: "pallet", id: palletId } });
    renderLabel(d as any);
  };
  const invalidateP = () => {
    qc.invalidateQueries({ queryKey: ["pallet", palletId] });
    qc.invalidateQueries({ queryKey: ["pallet-logs", palletId] });
  };
    ...
    <div className="mx-auto max-w-7xl space-y-4 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-slate-400">
            状态：{pallet.status} · 付款 {pallet.payment_status} · 创建 {fmtDate(pallet.created_at)}
            {pallet.batch_no ? ` · 批次 ${pallet.batch_no}` : ""} · 计费重{" "}
            <span className="font-mono text-amber-300">{pallet.chargeable_weight_kg ?? 0} kg</span>（
            {pallet.customer_code ? "自身" : "下属之和"}）
            {pallet.batch_status ? ` · 批次状态 ${pallet.batch_status}` : ""}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {pallet.batch_status && pallet.batch_status !== "draft" && (
            <button
              onClick={async () => {
                await setUnlock({ data: { kind: "pallet", id: palletId, unlocked: !pallet.unlocked } });
                invalidateP();
              }}
          ...
          <button
            onClick={async () => {
              if (confirm("删除此托盘？")) {
                await del({ data: { id: palletId } });
                navigate({ to: "/admin/pallets" });
              }
            }}
      ...
      {locked && (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200 inline-flex items-center gap-2">
          ...
          此托盘所属批次为 <b>{pallet.batch_status}</b>（非草稿），字段已锁定为只读。如需修改请点击右上角 "人工解锁"。
        </div>
      )}
      <ScanAddDialog
        open={showScan}
      ...
      <ContainerEditPanel
        ...
        locked={locked}
        onSave={async (patch) => {
          await updP({ data: { id: palletId, patch } });
          invalidateP();
        }}
      />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card title={`箱号 (${cartons.length})`}>
          <CartonCompactList
            ...
            onKick={async (c) => {
              await assign({ data: { palletId: null, cartonIds: [c.id] } });
              qc.invalidateQueries({ queryKey: ["pallet", palletId] });
            }}
          />
        </Card>
        ...
        <Card title={`直接挂载运单 (${waybills.length})`}>
          <WaybillCompactList
            ...
            onKick={async (w) => {
              await assign({ data: { palletId: null, waybillIds: [w.id] } });
              qc.invalidateQueries({ queryKey: ["pallet", palletId] });
            }}
          />
        </Card>
        ...
        <Card title={`订单 / 集运 (${orders.length + forwardings.length})`}>
          <ul className="space-y-1 text-xs">
            {orders.map((o: any) => (
              <li key={o.id}>
                订单{" "}
                <Link to="/admin/orders/$orderId" params={{ orderId: o.id }} className="font-mono text-brand">
                  {o.order_no}
                </Link>
              </li>
            ))}
            {forwardings.map((f: any) => (
              <li key={f.id}>
                集运{" "}
                <Link
      ...
      >
        ...
        {logsQ.data && logsQ.data.items.length === 0 && (
          <div className="py-4 text-center text-xs text-slate-500">暂无操作记录</div>
        )}
        {logsQ.data && logsQ.data.items.length > 0 && (
              ...
              <li key={l.id} className="flex items-start justify-between gap-3 py-2 text-xs">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 text-[10px] text-slate-500">
                    {l.operator_name ?? "系统"} · <span className="font-mono">{l.action}</span>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">加入运单</h2>
              <button onClick={() => setShowAddWb(false)}>
            ...
            <div className="max-h-96 overflow-y-auto rounded-lg border border-white/5">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-white/5">
                  {wbsQ.data?.waybills
                    .filter((w: any) => w.pallet_id !== palletId)
                    .map((w: any) => (
                      <tr key={w.id} className={selectedWb.has(w.id) ? "bg-brand/10" : ""}>
                        <td className="px-2 py-1.5">
                          <input
                            type="checkbox"
                            checked={selectedWb.has(w.id)}
                            onChange={() => {
                              const s = new Set(selectedWb);
      ...
      {showAssign && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">加入箱号</h2>
            ...
            <div className="max-h-96 overflow-y-auto rounded-lg border border-white/5">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-white/5">
                  {cartonsQ.data?.items
                    .filter((c: any) => c.pallet_id !== palletId)
                    .map((c: any) => (
