...
function ReceivingDetail() {
  ...
  const batchesQ = useQuery({
    queryKey: ["batches-for-recv-detail"],
  ...
  const onConfirm = async () => {
    ...
    if (hasMissing || hasPendingSecondary) {
      ...
      if (hasMissing)
        parts.push(
          `直挂未到 ${diff.missing_waybills.length} 单 / ${diff.missing_cartons.length} 箱 / ${diff.missing_pallets.length} 托`,
        );
  ...
  const onPrepare = async () => {
    ...
    try {
      ...
      alert(
        `已加入待派送：运单 ${res.counts.waybills} · 箱号 ${res.counts.cartons} · 托盘 ${res.counts.pallets}\n（新增 ${res.counts.inserted}，已存在 ${res.counts.skipped}）`,
      );
    } catch (e: any) {
      alert(e.message);
    } finally {
  ...
  // 直挂 stats
  const directWbScanned = direct.waybills.filter((w: any) =>
    scans.some((s: any) => s.kind === "waybill" && s.ref_id === w.id),
  ).length;
  const directCtScanned = direct.cartons.filter((c: any) =>
    scans.some((s: any) => s.kind === "carton" && s.ref_id === c.id),
  ).length;
  const directPlScanned = direct.pallets.filter((p: any) =>
    scans.some((s: any) => s.kind === "pallet" && s.ref_id === p.id),
  ).length;
    ...
    <div className="mx-auto max-w-7xl space-y-4 p-6">
        ...
        {r.batches ? (
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <span className="text-xs text-slate-400">
              {METHOD_LABEL[r.batches.shipping_method] ?? r.batches.shipping_method} · 发货{" "}
              {r.batches.planned_ship_date} · 目的地 {r.batches.destination_code ?? "—"}
            </span>
          </div>
        ) : (
          <div className="text-sm text-slate-400">
            尚未匹配批次（仅可匹配「已发货」状态的批次；扫描首个运单/箱号/托盘后会自动匹配）
          </div>
        )}
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          {isFinal ? (
            ...
            <>
              ...
              <p className="mt-1.5 text-[11px] text-slate-500">
                扫描箱号/托盘号仅确认外层；内部明细需进入下方「待二次扫描确认」逐件再次扫描。
              </p>
            </>
          )}
          <div className="mt-3 max-h-72 overflow-y-auto rounded-lg border border-white/5 bg-white/[0.02] p-2">
            {log.length === 0 ? (
              <div className="py-4 text-center text-xs text-slate-500">扫描记录将在这里实时显示</div>
            ) : (
              <ul className="space-y-1">
                {log.map((l, i) => (
                  <li
                    key={i}
                    className={`flex items-start gap-2 rounded-md px-2 py-1.5 text-xs ${!l.ok ? "bg-rose-500/10" : l.extra ? "bg-amber-500/10" : "bg-emerald-500/5"}`}
                  >
                    {!l.ok ? (
                    ...
                    <span
                      className={`flex-1 ${!l.ok ? "text-rose-200" : l.extra ? "text-amber-200" : "text-emerald-200"}`}
                    >
                      {l.info}
                    </span>
                  </li>
        ...
        <Card title="发货 vs 收货差异（直挂层）">
          {!r.batch_id ? (
            <div className="text-xs text-slate-500">匹配批次后将显示对比</div>
          ) : (
            <div className="space-y-3 text-sm">
              ...
              {(diff.missing_waybills.length > 0 ||
                diff.missing_cartons.length > 0 ||
                diff.missing_pallets.length > 0) && (
                <DiffBlock
                  title="未到 (Missing)"
                  cls="text-rose-300 bg-rose-500/10"
                  items={[
                    ...diff.missing_waybills.map(
                      (w: any) => `运单 ${w.waybill_no}${w.customer_code ? ` (${w.customer_code})` : ""}`,
                    ),
              ...
              {diff.extra_scans.length > 0 && (
                <DiffBlock
                  ...
                  items={diff.extra_scans.map(
                    (s: any) => `${s.kind === "waybill" ? "运单" : s.kind === "carton" ? "箱号" : "托盘"} ${s.code}`,
                  )}
                />
              )}
          ...
          title={
            (
              <span className="inline-flex items-center gap-2">
                待二次扫描确认
                <span className="text-xs text-slate-400">
                  （{secondary.total_count - secondary.pending_count}/{secondary.total_count}）
                </span>
                {secondaryAllDone && (
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {secondary.inner_cartons.length > 0 && (
              <SecondaryList
                ...
                items={secondary.inner_cartons.map((c: any) => ({
                  id: c.id,
                  label: c.carton_no,
                  sub: null,
                  scanned: c.scanned,
      ...
      {showMatch && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
          ...
          >
              ...
              {batchesQ.data?.batches
                ...
                .map((b: any) => (
                  <option key={b.id} value={b.id}>
                    {b.batch_no} · {METHOD_LABEL[b.shipping_method] ?? b.shipping_method} ·{" "}
            ...
            <button
              onClick={onMatch}
              className="mt-3 w-full rounded-md bg-brand py-2 text-sm font-semibold text-white hover:bg-brand/90"
            >
              保存
            </button>
          </div>
...
function ScanCol({
  ...
  return (
    <div className="rounded-lg border border-white/5 bg-white/[0.02]">
      <div className="border-b border-white/5 px-3 py-2 text-xs font-semibold text-slate-300">
        {title} <span className="text-slate-500">({items.length})</span>
      </div>
      {items.length === 0 ? (
        <div className="py-6 text-center text-[11px] text-slate-500">暂无</div>
      ) : (
        <ul className="max-h-80 divide-y divide-white/5 overflow-y-auto">
          {items.map((s: any) => (
            <li key={s.id} className="flex items-center justify-between px-3 py-1.5 text-xs hover:bg-white/[0.03]">
              ...
              {!isFinal && (
                <button onClick={() => onRemove(s.id)} className="text-rose-400 hover:text-rose-300">
...
function SecondaryList({
  ...
  title: string;
  items: { id: string; label: string; sub?: string | null; scanned: boolean }[];
}) {
  return (
    <div className="rounded-lg border border-white/5 bg-white/[0.02]">
      ...
      <ul className="max-h-72 divide-y divide-white/5 overflow-y-auto">
        {items.map((it) => (
          <li
            key={it.id}
            className={`flex items-center justify-between px-3 py-1.5 text-xs ${it.scanned ? "bg-emerald-500/5" : ""}`}
          >
...
function Stat({ label, sc, ex }: { label: string; sc: number; ex: number }) {
  ...
  return (
    <div
      className={`rounded-lg border p-2 ${ok ? "border-emerald-500/30 bg-emerald-500/5" : sc > ex ? "border-amber-500/30 bg-amber-500/5" : "border-white/10 bg-white/[0.02]"}`}
    >
