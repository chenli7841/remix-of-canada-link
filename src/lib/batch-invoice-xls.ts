...
const address = (party: any) =>
  [party?.name, party?.address, party?.country, party?.phone && `TEL: ${party.phone}`, party?.email]
    .filter(Boolean)
    .join("\n");
...
export function downloadBatchInvoiceWorkbook(data: any) {
  ...
  const transport = String(b.shipping_method ?? "")
    .toLowerCase()
    .includes("air")
    ? "Air"
    : "Sea";
  const detailRows = items.map((i: any) =>
    row(
      [
        cell(i.packages, "Number", "Integer"),
        cell(i.quantity, "Number", "Integer"),
        cell(i.net_weight_kg, "Number", "Decimal"),
        cell(i.cbm, "Number", "Volume"),
        cell("piece"),
        cell(i.name_en),
        cell(i.material, "String", i.material === "REVIEW" ? "Review" : "Cell"),
        cell(i.hs_code),
        cell(i.origin || "China"),
        cell(i.unit_price_cad, "Number", "Money"),
        cell("CAD"),
        cell(i.total_value_cad, "Number", "Money"),
      ],
      32,
    ),
  );
  ...
  const rows = [
    ...
    row([
      mergedCell("Vendor/Shipper/Exporter", 4, "Section"),
      cell(""),
      mergedCell("Ship Date", 1, "Label"),
      mergedCell(b.actual_ship_date ?? b.planned_ship_date, 3, "Value"),
    ]),
    row([
      mergedCell("Tax ID:", 1, "Label"),
      mergedCell(shipper.tax_id ?? "", 2, "Value"),
      cell(""),
      mergedCell("Mode of Transport", 1, "Label"),
      mergedCell(transport, 3, "Value"),
    ]),
    row([
      mergedCell("Contact Name", 1, "Label"),
      mergedCell(shipper.contact_name ?? "", 2, "Value"),
      cell(""),
      mergedCell("Tracking No.", 1, "Label"),
      mergedCell(b.batch_no, 3, "Value"),
    ]),
    row([
      mergedCell("Contact phone", 1, "Label"),
      mergedCell(shipper.phone ?? "", 2, "Value"),
      cell(""),
      mergedCell("Invoice No.:", 1, "Label"),
      mergedCell(b.batch_no, 3, "Value"),
    ]),
    row([
      mergedCell("Email", 1, "Label"),
      mergedCell(shipper.email ?? "", 2, "Value"),
      cell(""),
      mergedCell("Purchase Order No.:", 1, "Label"),
      mergedCell("", 3, "Value"),
    ]),
    row([
      mergedCell("Company Name", 1, "Label"),
      mergedCell(shipper.name ?? "", 2, "Value"),
      cell(""),
      mergedCell("Payment Terms:", 1, "Label"),
      mergedCell("", 3, "Value"),
    ]),
    row(
      [
        mergedCell("Company address", 1, "Label"),
        mergedCell(shipper.address ?? "", 2, "Value"),
        cell(""),
        mergedCell("Shipping Term", 1, "Label"),
        mergedCell("", 3, "Value"),
      ],
      34,
    ),
    row([
      mergedCell("Country/Territory", 1, "Label"),
      mergedCell(shipper.country ?? "China", 2, "Value"),
      cell(""),
      mergedCell("Bill of Lading:", 1, "Label"),
      mergedCell(b.vessel_no ?? "", 3, "Value"),
    ]),
    row([
      mergedCell("", 4, "Value"),
      cell(""),
      mergedCell("Purpose of Shipment", 1, "Label"),
      mergedCell("", 3, "Value"),
    ]),
    row([
      mergedCell("CONSIGNEE", 4, "Section"),
      cell(""),
      mergedCell("SOLD TO / IMPORTER (if different from Consignee):", 4, "Section"),
    ]),
    ...
    row(
      [
        ...
        "UNIT VALUE",
        "CURRENCY OF VALUE",
        "TOTAL VALUE (QTY OF UNITS * UNIT VALUE)",
      ].map((x) => cell(x, "String", "TableHeader")),
      58,
    ),
    ...detailRows,
    row([
      cell(+totalPackages.toFixed(2), "Number", "Total"),
      cell(+totalQty.toFixed(0), "Number", "Total"),
      cell(+totalNet.toFixed(2), "Number", "Total"),
      cell(+totalCbm.toFixed(3), "Number", "Total"),
      cell(""),
      ...
      cell(""),
      cell(""),
      cell("CAD", "String", "Total"),
      cell(+totalValue.toFixed(2), "Number", "TotalMoney"),
    ]),
    row([
      mergedCell("TOTAL PACKAGE", 1, "FooterLabel"),
      mergedCell("TOTAL NET WEIGHT", 1, "FooterLabel"),
      mergedCell("TOTAL GROSS WEIGHT", 1, "FooterLabel"),
      mergedCell("TOTAL CBM", 1, "FooterLabel"),
      mergedCell("HBL / CONTAINER", 3, "FooterLabel"),
    ]),
    row([
      mergedCell(+totalPackages.toFixed(2), 1, "FooterValue"),
      mergedCell(`${totalNet.toFixed(2)} kgs`, 1, "FooterValue"),
      mergedCell(
        `${Number(adjustment.target_gross_kg ?? b.hbl_total_weight_kg ?? 0).toFixed(2)} kgs`,
        1,
        "FooterValue",
      ),
      mergedCell(`${Number(adjustment.target_cbm ?? b.hbl_total_volume_m3 ?? 0).toFixed(3)} m³`, 1, "FooterValue"),
      mergedCell(b.container_no ?? "", 3, "FooterValue"),
    ]),
  ];
