...
export default defineTool({
  ...
  description:
    "Owner-only mapping of the existing EPLUS admin basic-info edit. Review the latest order and obtain explicit confirmation first.",
  inputSchema: {
    ...
    patch: z.object({
      ...
      domestic_tracking_no: z.string().max(200).optional(),
      intl_tracking_no: z.string().max(200).optional(),
    }),
    reason: z.string().min(2).max(500),
    confirmation: z.literal("CONFIRM_UPDATE_FORWARDING"),
  },
  ...
  handler: async (input, ctx) => {
    ...
    if (error) {
      console.error("MCP update_forwarding_basic_info_owner failed", { code: error.code });
