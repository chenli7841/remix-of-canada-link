...
export default defineTool({
  ...
  description:
    "Get one forwarding order belonging to the signed-in customer, including items and waybills. All money is returned in CAD. This tool cannot pay.",
  ...
  handler: async ({ forwarding_id }, ctx) => {
    ...
    const [{ data: itemRows, error: itemError }, { data: waybillRows, error: waybillError }] = await Promise.all([
      sb
        ...
        .select("id,name,quantity,hs_code,unit_price_cad,extras,created_at")
        .eq("forwarding_id", forwarding_id)
        .order("created_at"),
      sb
        .from("waybills")
        .select(
