...
export default defineTool({
  ...
  description:
    "List and count forwarding orders belonging to the currently signed-in EPLUS account. OAuth already identifies and scopes the customer: call this tool directly for requests such as '我的运单/集运单' or '我有多少运单', and never ask for a customer code, phone number, WeChat ID, or another identity value. Filters are optional; an unqualified count means all of the signed-in user's forwarding orders. Returns total_count plus a small page; ask before loading more. All money is CAD.",
  inputSchema: {
    request_no: z.string().max(100).optional(),
  ...
  handler: async ({ request_no, status, date_from, date_to, offset, limit }, ctx) => {
    ...
    if (request_no) query = query.ilike("request_no", `%${request_no}%`);
    ...
    const result = {
      currency: "CAD",
      total_count: count ?? forwardings.length,
      forwardings,
