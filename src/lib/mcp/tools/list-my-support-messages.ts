...
export default defineTool({
  ...
  description: "List support messages belonging to the signed-in EPLUS customer, newest first, and mark the account's support messages as read. This updates read receipts and the unread count. Never accept a customer code from chat.",
  inputSchema: { limit: z.number().int().min(1).max(50).optional() }, annotations: { readOnlyHint: false, idempotentHint: true, openWorldHint: false, destructiveHint: false },
