...
export default defineTool({
  ...
  inputSchema: {
    month: z
      ...
      .optional()
      .describe("Planned shipping month in YYYY-MM, using the customer's intended year."),
  ...
  handler: async ({ month, limit }, ctx) => {
    ...
    try {
      ...
      if (!batchIds.length)
        return { content: [{ type: "text", text: "[]" }], structuredContent: { currency: "CAD", batches: [] } };
      let batchQuery = supabaseAdmin
        .from("batches")
        .select("id,batch_no,status,shipping_method,planned_ship_date,eta_date")
        .in("id", batchIds)
      ...
      for (const b of batchRows ?? []) {
        ...
        batches.push({
          ...
          eta: b.eta_date,
          subtotal_cad: priceConfirmed ? subtotalCad : null,
          ...
          waybills: mineWbs.map((w: any) => ({
            waybill_no: w.waybill_no,
            status: w.status,
            payment_status: w.payment_status,
            intl_tracking_no: w.intl_tracking_no,
          })),
        });
      }
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { currency: "CAD", month: month ?? null, match_count: batches.length, batches },
              null,
              2,
            ),
          },
        ],
        structuredContent: { currency: "CAD", month: month ?? null, match_count: batches.length, batches },
      };
    } catch (error) {
