...
export default defineTool({
  ...
  inputSchema: {
    ...
    expected_version: z
      ...
      .int()
      .positive()
      .describe("Version returned by the immediately preceding get_forwarding_draft review."),
    confirmation: z
      .literal("CONFIRM")
      .describe("Must be CONFIRM after the customer explicitly approves the final summary."),
  },
  annotations: { readOnlyHint: false, idempotentHint: true, destructiveHint: false, openWorldHint: false },
  handler: async ({ draft_id, expected_version }, ctx) => {
    if (!ctx.isAuthenticated()) return unauthenticatedResult();
    const { data, error } = await supabaseForUser(ctx).rpc("confirm_ai_forwarding_draft", {
      _draft_id: draft_id,
