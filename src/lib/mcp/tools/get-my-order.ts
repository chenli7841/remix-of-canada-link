...
export default defineTool({
  ...
  description:
    "Get one shop order belonging to the signed-in customer, including items and waybills. All money is returned in CAD. This tool cannot pay.",
  ...
  handler: async ({ order_id }, ctx) => {
    ...
    const { data: order, error: orderError } = await sb
      .from("orders")
      .select(
        "id,order_no,status,payment_status,created_at,shipping_method,tracking_no,domestic_tracking_no,intl_tracking_no,batch_no,eta,address_snapshot,note,customer_code,destination_code,route_code,company_code,box_no,pallet_no,box_count,subtotal_cny,shipping_cny,insurance_cny,customs_cny,total_cny,fx_rate",
      )
      .eq("id", order_id)
      .maybeSingle();
    ...
    const [{ data: itemRows, error: itemError }, { data: waybillRows, error: waybillError }] = await Promise.all([
      sb
        .from("order_items")
        .select("id,name_zh,name_en,sku,quantity,paid,subtotal_cny,unit_price_cny,waybill_id")
        .eq("order_id", order_id)
        .order("created_at"),
      sb
        .from("waybills")
        .select(
          "id,waybill_no,status,shipping_method,weight_kg,length_cm,width_cm,height_cm,eta,intl_tracking_no,batch_no,box_no,pallet_no,items_summary,freight_cad,duty_cad,clearance_cad,insurance_cad,surcharge_cad,created_at,updated_at",
        )
        .eq("order_id", order_id)
        .order("created_at"),
    ]);
