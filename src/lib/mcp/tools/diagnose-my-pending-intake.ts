...
export default defineTool({
  ...
  handler: async ({ tracking_no }, ctx) => {
    ...
    const { data, error } = await supabaseForUser(ctx).rpc("chatgpt_diagnose_my_pending_intake", {
      _tracking_no: tracking_no,
    });
