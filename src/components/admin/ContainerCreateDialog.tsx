...
export function ContainerCreateDialog({
  ...
  const filteredWarehouses = useMemo(() => {
    ...
    if (selectedRoute.origin?.code)
      return all.filter((w: any) => w.can_origin && w.country === selectedRoute.origin?.country);
      ...
      >
        ...
        <div className="rounded-md border border-white/5 bg-white/[0.03] px-3 py-2 text-[11px] text-slate-400">
          <div>
            ...
            <span className="font-mono text-slate-200">
              {prefix} + 日期(YYYYMMDD) + [线路] + [客户号] + [取货点] + [目的地] + 序号(3位)
            </span>
          </div>
          <div className="mt-1">
            预览：<span className="font-mono text-amber-300">{previewNo}</span>
          </div>
        </div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <label className="text-xs text-slate-400">
            客户号（搜索）
            <input
              value={custQuery || form.customer_code}
              onChange={(e) => {
                setCustQuery(e.target.value);
                setForm({ ...form, customer_code: e.target.value, customer_user_id: "" });
              }}
              ...
              <div className="mt-1 max-h-32 overflow-y-auto rounded-md border border-white/10 bg-[#0E1626]">
                {usersQ.data.users.map((u: any) => (
                  <button
                    ...
                    key={u.id}
                    onClick={() => {
                      setForm({ ...form, customer_user_id: u.id, customer_code: u.customer_code ?? "" });
                      setCustQuery("");
                    }}
          ...
          <label className="text-xs text-slate-400">
            取货点（仓库）
            <select
              value={form.pickup_warehouse}
              onChange={(e) => setForm({ ...form, pickup_warehouse: e.target.value })}
              className="mt-1 w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-sm text-slate-100 [&>option]:bg-[#0E1626]"
            >
              <option value="">— 不限 —</option>
