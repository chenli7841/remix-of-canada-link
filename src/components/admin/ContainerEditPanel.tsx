...
export function ContainerEditPanel({
  ...
  return (
    <Card
      title={`基本字段 · ${kind === "carton" ? "箱号" : "托盘"}线路/状态/客户/地点`}
      action={
        <button
          onClick={handleSave}
    ...
    >
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6 text-xs">
        ...
        <F
          label="线路代码"
          value={f.route_code}
          onChange={upd("route_code")}
          placeholder="例如 CAAIR"
          disabled={locked}
        />
        <div>
          ...
          >
            {CARTON_STATUS.map((s) => (
              <option key={s} value={s} className="bg-slate-700 text-white">
                {s}
              </option>
            ))}
          </select>
        ...
        <F
          ...
          onChange={upd("customer_code")}
          placeholder="5 位客户号"
          disabled={locked}
        />
        <F
          label="取货点"
          value={f.pickup_warehouse}
          onChange={upd("pickup_warehouse")}
          placeholder="仓库名"
          disabled={locked}
        />
        <F
