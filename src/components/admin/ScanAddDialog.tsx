  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
      <div
        ...
        className="w-full max-w-xl rounded-2xl border border-white/10 bg-[#0A0F1A] p-5"
      >
        <div className="mb-3 flex items-center justify-between">
          <h2 className="font-display text-lg font-bold inline-flex items-center gap-2">
            ...
            扫码加入 ({container === "batch" ? "批次" : container === "carton" ? "箱号" : "托盘"})
          </h2>
        ...
        <p className="mb-3 text-xs text-slate-400">
          连续扫描运单号 / 箱号 (BOX...) / 托盘号 (PAL...)，系统自动识别。回车提交。
        </p>
          ...
          {log.length === 0 ? (
            ...
            <ul className="space-y-1">
              {log.map((l, i) => (
                <li
                  key={i}
                  className={`flex items-start gap-2 rounded-md px-2 py-1.5 text-xs ${l.ok ? "bg-emerald-500/5" : "bg-rose-500/10"}`}
                >
                  {l.ok ? (
        ...
        <div className="mt-3 flex justify-between text-[11px] text-slate-500">
          <div>
            共 {log.length} 条记录 · ✓ {log.filter((l) => l.ok).length} · ✗ {log.filter((l) => !l.ok).length}
          </div>
          <button onClick={() => setLog([])} className="text-slate-400 hover:text-white">
            清空
          </button>
        </div>
      </div>
